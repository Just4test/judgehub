...
S3_CFN_PACKAGE_BUCKET_NAME ?= cfnpackagetemp
...
package: 
	cd src; \
	dep2layer download; \
	sam package --template-file .dep2layer-template.yaml --output-template-file .packaged-template.yaml --s3-bucket $(S3_CFN_PACKAGE_BUCKET_NAME)

.PHONY: deploy
deploy:
	cd src; \
	aws cloudformation deploy --template-file .packaged-template.yaml  --capabilities CAPABILITY_IAM --stack-name $(CFN_STACK_NAME) --parameter-overrides UseCurrentS3Bucket=$(S3_BUCKET_NAME) JudgeVpcCidrBlock=10.97